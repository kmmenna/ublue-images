...
# --- Broadcom WiFi (ex: BCM4360) proprietary driver (akmod-wl from RPM Fusion nonfree) ---

# 1. Install akmods to create the 'akmods' user (not intended to run as root)
...
# 4. Determine the installed image kernel version (use the version of installed kernel-devel, not the host)
KERNEL_RELEASE=$(rpm -q kernel-devel --qf '%{version}-%{release}.%{arch}\n' | head -1)

# 5. Build the Broadcom wl kernel module RPM as the 'akmods' user, then install it as root:
