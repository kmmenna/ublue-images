...
#
# Usage: get-next-tag-number.sh <package_name> <date_prefix> [token]
...
PACKAGE_NAME="${1:?Missing package_name}"
DATE_PREFIX="${2:?Missing date_prefix}"
TOKEN="${3:-${GITHUB_TOKEN:-}}"

if [[ -z "$TOKEN" ]]; then
  echo "Error: GITHUB_TOKEN must be set or passed as 3rd argument" >&2
  exit 1
fi

# List package versions: GET /user/packages/container/{name}/versions
# Each version has a "name" field (the tag) and optionally metadata.container_tags.
# Uses authenticated user only (no org).
